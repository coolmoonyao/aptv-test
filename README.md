...
| 规则 | 说明 |
|---|---|
| 白名单分组 | 今日影视、港澳台、咪视界、地方台、4GTV直播1、央卫视直播1 —— 这 6 组整组保留（源里没有则跳过）；**能确认分辨率 <1080 则去除，无法确认则默认保留**（含 403/404/翻墙/专网等脚本机探测不到、但手机端实测可播的源） |
| 其余节目 | 分辨率 ≥ 1080P（1920×1080）且 HTTP 首字节耗时 ≤ 2000 ms |
| 全球节目 | 其余节目中，美国 / 日本 / 韩国 / 英国 / 法国 / 新加坡 归并为「全球节目」分类 |
| 排除项 | 移动专网、内网、4GTV直播2、抖音/虎牙/斗鱼/快手、Twitch/TikTok、点播/回放/合集、动漫/游戏/少儿/电视剧/综艺/体育/足球、音乐/歌曲/解说、埋堆堆等 |
| 同名合并 | 同名节目合并为多链接备用源；分辨率降序，同分辨率按响应时间升序 |

> 说明：分辨率与响应时间通过本机 `ffprobe` 真实探测获得（源列表不含分辨率字段）。白名单分组「无法确认分辨率」会**默认保留**（疑罪从无），因为不少源（移动专网、翻墙、特定 UA 防盗链）在脚本机上探测不到，但用户手机端实测可播；确认 <1080 才去除。
...
- `keep_groups`：白名单分组数组（子串匹配 group-title）。命中的分组不参与 include/exclude 关键词过滤；画质口径为「能确认分辨率 < min_width×min_height 则去除，无法确认则保留」（死链仍剔除）。
- `merge_international`：国际频道归并配置。`group_match` 按分组名、`name_match` 按频道名兜底，命中的频道统一改写为 `group_name`（默认「国际直播」）。
